...
const isContatoSalvo = async (msg) => {
  try {
    ...
    // 1️⃣ Verifica primeiro no seu sistema
    const clientes = carregarClientes();
    if (clientes[numero]) {
      console.log(`[v0] [CLIENTE CADASTRADO] ${clientes[numero].nome}`);
      return true;
    }

    // 2️⃣ Fallback seguro (SEM isMyContact)
    ...
    if (nomeValido) {
      console.log(`[v0] [CONTATO EXISTENTE] ${contact.name || contact.pushname}`);
    }

    return nomeValido;
  } catch (error) {
    console.error("[ERRO] Falha ao verificar contato:", error);
    return false;
  }
};
